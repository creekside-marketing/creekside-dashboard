...
export default function ClientTable() {
  ...
  const activeGroups = useMemo(() =>
    grouped
      .map(g => ({ ...g, rows: g.rows.filter(r => (r.client_category ?? 'active') !== 'retainer' && r.platform !== 'other') }))
  ...
  // Summary stats
  const stats = useMemo(() => {
    const activeFiltered = filtered.filter(c => (c.client_category ?? 'active') !== 'retainer' && c.platform !== 'other');
