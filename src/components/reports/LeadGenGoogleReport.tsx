...
export default function LeadGenGoogleReport({
  client,
  mode,
  bookedConversionActions,
}: {
  client: ReportingClient;
  mode: 'internal' | 'public';
  /**
   * Google Ads conversion action names to sum into the "Booked Consultations"
   * panel. Omitted/empty for most clients, which hides the panel entirely.
   */
  bookedConversionActions?: string[];
}) {
  const data = useGoogleAdsData(client.ad_account_id);
  const {
    campaigns, totals, dailyData, keywords, searchTerms,
    geoData, ageData, genderData, kpiChanges, conversionBreakdown,
  ...
  // Booked outcomes — opt-in per client via booked_conversion_actions.
  const bookedActions = bookedConversionActions ?? [];
  const bookedCount = bookedActions.length > 0
    ? conversionBreakdown
        .filter((e) => bookedActions.includes(e.name))
        .reduce((sum, e) => sum + Number(e.conversions ?? 0), 0)
    : 0;
    ...
    <div className="space-y-6">
      ...
      {!loading && !error && (
        <>
          ...
          {/* 2b. Booked Consultations — renders only for clients configured
              with booked_conversion_actions. */}
          {bookedActions.length > 0 && (
