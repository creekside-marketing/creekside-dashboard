...
/**
 * RetirementIncomeSolutionsMetaReport — Custom Meta Ads report for
 * Retirement Income Solutions (client id d8231c09; ad account act_1423698879754960).
 *
 * Branched from the shared LeadGenMetaReport template. Edits here affect ONLY
...
import { SparklineKpiCard, getChangeColor } from '../shared';
...
export default function RetirementIncomeSolutionsMetaReport({ client, mode, leadConversionTypes, pqlConversionType, hideReferralBanner, controlledPeriod }: { client: ReportingClient; mode: 'internal' | 'public'; leadConversionTypes?: string[]; pqlConversionType?: string; hideReferralBanner?: boolean; controlledPeriod?: PriorPeriodDates }) {
  ...
  /**
   * Campaigns that actually ran in the period, ordered by lead volume, with each
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    {c.change.pct !== '--' && (
                      <span className="text-xs font-medium tabular-nums ml-auto"
                        ...
                        {c.change.direction === 'up' ? '\u2191' : c.change.direction === 'down' ? '\u2193' : ''}{c.change.pct}
                      </span>
                    )}
                  </div>
