...
/**
 * QuivrMeta — Custom report for Quivr (Meta Ads).
 *
 * Branched from LeadGenMetaReport. Adds a Schedules column to the Campaign
 * Performance and Ads Overview tables, pulled from Meta's conversions array
 * (action_type schedule_total) since Quivr tracks scheduled demos on the site.
 *
 * CANNOT: Modify ad account data — read-only display.
 * CANNOT: Affect other clients — this file only renders for Quivr.
 */
...
interface LeadGenRow {
  ...
  leads: number; schedules: number; reach: number; frequency: number;
  cpm: number; cpl: number; lctr: number;
}
...
// Quivr-specific: schedules live in the conversions array (schedule_total).
// We prefer schedule_total since schedule_website is a subset that usually matches.
function getSchedules(conversions: MetaAction[] | undefined): number {
  if (!conversions) return 0;
  const total = conversions.find((a) => a.action_type === 'schedule_total');
  if (total) return Math.round(Number(total.value) || 0);
  const website = conversions.find((a) => a.action_type === 'schedule_website');
  return website ? Math.round(Number(website.value) || 0) : 0;
}
...
function normalize(row: any): LeadGenRow {
  ...
  const schedules = getSchedules(conversions);
  const reach = Number(row.reach ?? 0);
  return {
    name: row.campaign_name ?? row.adset_name ?? row.ad_name ?? 'Unknown',
    impressions, linkClicks, spend, leads, schedules, reach,
...
function computeTotals(rows: LeadGenRow[]): Omit<LeadGenRow, 'name'> {
  const s = rows.reduce((a, c) => ({
    ...
    spend: a.spend + c.spend, leads: a.leads + c.leads, schedules: a.schedules + c.schedules,
    reach: a.reach + c.reach,
  }), { impressions: 0, linkClicks: 0, spend: 0, leads: 0, schedules: 0, reach: 0 });
...
function parseDailyRows(rows: any[]): DailyRow[] {
  return rows.map((row) => {
    ...
    const schedules = getSchedules(conversions);
    const reach = Number(row.reach ?? 0);
    return { date: row.date_start ?? row.date ?? '', impressions, linkClicks, spend, leads, schedules, reach,
      frequency: reach > 0 ? impressions / reach : 0,
      cpm: impressions > 0 ? (spend / impressions) * 1000 : 0,
      cpl: leads > 0 ? spend / leads : 0,
      lctr: impressions > 0 ? linkClicks / impressions : 0 };
...
const ZERO: Omit<LeadGenRow, 'name'> = { impressions: 0, linkClicks: 0, spend: 0, leads: 0, schedules: 0, reach: 0, frequency: 0, cpm: 0, cpl: 0, lctr: 0 };

// ── Component ────────────────────────────────────────────────────────────

export default function QuivrMeta({ client, mode }: { client: ReportingClient; mode: 'internal' | 'public' }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && !error && (<>
        ...
        {/* 6. Campaign Performance — Quivr adds a Schedules column */}
        ...
        {adsData.length > 0 && (() => {
          ...
          }).sort((a, b) => (b.schedules - a.schedules) || (b.leads - a.leads)).slice(0, 15);
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-200 bg-slate-50/80">
                      ...
                      <th className="text-xs font-semibold text-slate-500 uppercase tracking-wider py-3 px-4 text-right">Schedules</th>
                    </tr>
                  </thead>
                  <tbody>
                    {processedAds.map((ad, idx) => {
                      ...
                      return (
                        <tr key={idx} className="border-b border-slate-100 hover:bg-blue-50/50 transition-colors">
                          ...
                          <td className="text-sm text-right text-slate-700 py-3 px-4 tabular-nums">{fmt(ad.schedules)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
